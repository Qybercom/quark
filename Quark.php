...
class QuarkFile implements IQuarkModel, IQuarkStrongModel, IQuarkLinkedModel {
	...
	private function _followParent ($mode = self::MODE_DEFAULT) {
		...
		$mask = umask(0);
		$ok = @mkdir($this->parent, self::MODE_ANYONE/*$mode == self::MODE_DEFAULT ? $mask : $mode*/, true);
		umask($mask);
		
		if ($mask != umask())
			Quark::Log('[QuarkFile::_followParent] The [umask:' . $mask . '] did not restored correctly for "' . $this->parent . '". Current: ' . umask());
